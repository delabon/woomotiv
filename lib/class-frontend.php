...
class Frontend
{
    ...
    private function replace_shortcodes($template_content, $notification)
    {
        ...
        $template_content = str_ireplace(
            '{date}',
            '<span class="wmt-date">' . $notification['popup']['date_completed'] . '</span>',
            $template_content
        );
        $template_content = str_ireplace('{new_line}', '<br>', $template_content);
        $template_content = str_ireplace(
            '{buyer}',
            '<strong class="wmt-buyer">' . $notification['popup']['user']['first_name'] . ' ' . $notification['popup']['user']['last_name'] . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{product}',
            '<strong class="wmt-product">' . $notification['popup']['product']['name'] . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{by_woomotiv}',
            '<strong class="wmt-by">by <span>woomotiv</span></strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{city}',
            '<strong class="wmt-city">' . $city . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{country}',
            '<strong class="wmt-country">' . $country . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{state}',
            '<strong class="wmt-state">' . $state . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{buyer_first_name}',
            '<strong class="wmt-buyer-first-name">' . $notification['popup']['user']['first_name'] . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{buyer_last_name}',
            '<strong class="wmt-buyer-last-name">' . $notification['popup']['user']['last_name'] . '</strong>',
            $template_content
        );
        $template_content = str_ireplace(
            '{buyer_username}',
            '<strong class="wmt-buyer-username">' . $notification['popup']['user']['username'] . '</strong>',
            $template_content
        );
    ...
    function ajax_update_stats()
    {
        ...
        if ($type === 'review' || $type === 'product') {
            ...
            $stats = $wpdb->get_row(
                "SELECT * FROM {$wpdb->prefix}woomotiv_stats 
